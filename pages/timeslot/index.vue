  ...
  <div id="app">
    <section class="myContainer container columns is-multiline is-centered">
      ...
      >
        <b-tab-item
          :label="'Page ' + i"
          v-for="i in Math.ceil(dataList.length / 8)"
          :key="i"
          class="columns is-multiline is-centered myTabItem"
        >
          <div
            v-for="timeslot in dataList.slice((i-1)*8, i*8)"
            :key="timeslot.session_id"
            class="column is-one-quarter"
            :class="{selectedCard : selectedIndex == timeslot.session_id}"
          >
            <div
              class="card myCard"
              @click="selectTimeSlot(timeslot, timeslot.session_id)"
              :class="{disabledCard: isDisabled(timeslot.capacity - timeslot.noBooked, timeslot.session_id)}"
            >
              <div class="media fullHeight">
                <div class="media-left fullHeight">
                  <div
                    class="centerTextBox"
                    v-if="timeslot.session_id != selectedTimeSlot.session_id"
                  >
                    <p
                      class="has-text-weight-bold is-size-4 has-text-centered"
                    >{{timeslot.capacity - timeslot.noBooked}}</p>
                  </div>
                  <div
                    class="centerTextBox"
                    v-if="timeslot.session_id != selectedTimeSlot.session_id"
                  ...
                  <div
                    class="centerTextBox fullHeight"
                    v-if="timeslot.session_id == selectedTimeSlot.session_id"
                  >
                ...
                <div class="media-content centerTextBox fullHeight">
                  <p
                    ...
                    :class="{selectedCard : selectedIndex == timeslot.session_id}"
                  >
                    {{timeslot.session_start}}
                    <br>-
                    <br>
                    {{timeslot.session_end}}
                  </p>
                </div>
              </div>
            </div>
      ...
      <div class="column is-half level" v-else>
        <p class="level-item has-text-weight-bold is-size-2 has-text-grey">
          There are
          <b>&nbsp;no slots&nbsp;</b> left for the day.
        </p>
      </div>
    </section>
  </div>
...
export default {
  ...
  methods: {
    ...
    makeBooking() {
      let check = true;
      const prevBooking = this.bookingDetail;
      ...
      if (this.disabled === false && check) {
        ...
        this.$dialog.alert({
          title: "Error",
          message:
            "This is your current time slot.<br />PleeZ select a <b>different</b> one.",
          confirmText: "Ok",
          size: "is-large",
          type: "is-danger"
        });
  ...
  async beforeMount() {
    const roleId = this.bookingCart.role;
    const res = await this.$axios.$get(`/sessions/getSessionList/${roleId}`);
    this.dataList = res;
